...
<div id="ransomScreen" role="dialog" aria-modal="true" aria-hidden="true">
  <div class="ransom-wrap">
    ...
    <div class="ransom-desc">
      Sistem anda telah kami bongkar. Data penting, database, dan semua file pribadi sudah kami kunci.<br>
      <strong>Kami bukan hacker amatiran.</strong> Hanya butuh 10 detik untuk menghancurkan reputasi anda.<br>
      <span style="color:#ff5252;">Bayar tebusan atau lihat semua file anda lenyap.</span>
    </div>

    <div class="wallet-box">
      <span style="color:#ffcc00;">▶ PULSA/OVO/DANA:</span><br>
      <code style="font-size:20px;">6289517848185</code>
    </div>
...
btnContact.addEventListener('click', () => {
  showModal('Hubungi Hacker', 'WA: 6289517848185\nTelegram: @UpinXD\nJangan coba-coba lapor polisi.');
});
btnInfo.addEventListener('click', () => {
  showModal('Info Hacker', 'Team @UpinXD — Spesialis deface & ransomware. Kami punya backdoor di sistem anda.');
});
modalClose.addEventListener('click', closeModal);
btnPay.addEventListener('click', () => {
  showModal('Bayar Tebusan', 'Kirim ke 6289517848185 (Pulsa/OVO/DANA). Batas waktu 10 menit.');
});
btnHubungi.addEventListener('click', () => {
  showModal('Hubungi @UpinXD', 'WA: 6289517848185\nTelegram: @UpinXD');
});
btnBackFromRansom.addEventListener('click', exitRansom);

const fakeFiles = [
  "C:\\Users\\Admin\\Documents\\database_karyawan.xlsx",
  "C:\\Users\\Admin\\Pictures\\foto_ktp.jpg",
  "C:\\Users\\Admin\\Desktop\\rahasia_perusahaan.txt",
  "C:\\Users\\Admin\\Videos\\meeting_rahasia.mp4",
  "D:\\Project\\source_code\\app.php",
  "C:\\Users\\Admin\\Downloads\\password_list.csv",
  "/home/admin/backup/db_production.sql",
  "/var/www/html/config.php",
  "/etc/shadow",
  "/root/.ssh/id_rsa",
  "E:\\Backup\\2025\\data_penting.zip"
];
...
function startCountdown(seconds){
  ...
  function tick(){
    if(remaining < 0){
      clearInterval(countdownInterval);
      countdownEl.textContent = "⌛ WAKTU HABIS! FILE TERHAPUS.";
      return;
    }
...
function positionVideoRandom(){
  ...
  const elW = videoContainer.offsetWidth || 360;
  const elH = videoContainer.offsetHeight || 202;
  const margin = 24;
  const maxX = Math.max(margin, vw - elW - margin);
  const maxY = Math.max(margin, vh - elH - margin);
  videoContainer.style.left = (Math.floor(Math.random() * (maxX - margin + 1)) + margin) + 'px';
  videoContainer.style.top = (Math.floor(Math.random() * (maxY - margin + 1)) + margin) + 'px';
}

function startVideoRandomMovement(){
  clearInterval(videoMoveInterval);
  videoMoveInterval = setInterval(positionVideoRandom, 3000 + Math.floor(Math.random()*1500));
}
...
window.addEventListener('load', () => {
  videoContainer.style.left = (window.innerWidth - 360 - 24) + 'px';
  videoContainer.style.top = '24px';
});

window.addEventListener('resize', () => {
  if(videoContainer.style.display === 'block') positionVideoRandom();
});
</script>
</body>
</html>
